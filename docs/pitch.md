...
Cero: el resumen semántico corre embebido dentro del propio CLI (`node-llama-cpp` + Qwen2.5-Coder 1.5B) — sin API key, sin costo por token, sin depender de internet salvo la descarga única del modelo (~1GB) que `contextcore init` hace sola, y sin ningún servicio externo que instalar o mantener corriendo (nada de Ollama ni Python). Y si esa descarga falla o la plataforma no tiene binario compatible, hay un fallback sin costo (mensaje de commit + trailers `decision:`/`gotcha:`) que sigue siendo mejor que nada.
